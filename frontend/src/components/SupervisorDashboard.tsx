...
const STATUS_COLORS: Record<string, string> = {
  ...
  'On Review': 'bg-amber-500/20 text-amber-400',
  'Rework': 'bg-red-500/20 text-red-400',
    ...
    <div className={`h-screen w-screen p-6 flex flex-col font-sans transition-colors ${bg}`}>
      <header className={`flex justify-between items-center mb-4 border-b pb-4 transition-colors ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
        <div>
          ...
          <p className="text-xs opacity-50">Контроль проекта, распределение задач, аналитика</p>
        </div>
        ...
        <div className={`flex-1 border rounded-2xl p-4 flex flex-col min-h-0 transition-all ${card}`}>
          <div className="flex justify-between items-center mb-3 shrink-0">
            ...
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              className={`text-xs border rounded px-2 py-1 focus:outline-none ${darkMode ? 'bg-black border-gray-800 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
            >
              ...
              <div
                ...
                className={`p-3 border rounded-xl cursor-pointer flex justify-between items-center transition-all ${
                  selectedTask?.id === t.id
                    ? (darkMode ? 'border-purple-500 bg-purple-500/10' : 'border-purple-600 bg-purple-50')
                    : (darkMode ? 'bg-black/40 border-gray-800 hover:border-gray-700' : 'bg-gray-50 border-gray-200 hover:border-gray-300')
                }`}
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] opacity-50">| {t.duration} мин</span>
                    <span className="text-[10px] opacity-50">| до {t.deadline}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 ml-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${STATUS_COLORS[t.status] || ''}`}>
                  ...
                  <select
                    ...
                    className={`text-[10px] border rounded px-1 py-0.5 focus:outline-none ${darkMode ? 'bg-black border-gray-700 text-white' : 'bg-white border-gray-300'}`}
                  >
          ...
          {selectedTask ? (
            <>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div><span className="opacity-50">Длительность:</span> <span>{selectedTask.duration} мин</span></div>
                ...
                <button
                  onClick={() => alert('Задача назначена/перераспределена')}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold py-2 rounded-lg transition-colors mt-2"
                >
