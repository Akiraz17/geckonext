...
function h(t2, e2 = {}) {
  ...
  const c = (e3) => {
    ...
    let l2 = e3.clientX, c2 = e3.clientY, u = false;
    const v = Date.now(), p = t2.getBoundingClientRect(), { left: g, top: m } = p, f = (t3) => {
      if (t3.pointerId !== s2) return;
      if (t3.defaultPrevented || h2.size > 1) return;
      if (a2 && Date.now() - v < r2) return;
      const e4 = t3.clientX, n3 = t3.clientY, d3 = e4 - l2, p2 = n3 - c2;
      (u || Math.abs(d3) > i2 || Math.abs(p2) > i2) && (t3.preventDefault(), t3.stopPropagation(), u || (o2.set({ type: "start", x: l2 - g, y: c2 - m }), u = true), o2.set({ type: "move", x: e4 - g, y: n3 - m, deltaX: d3, deltaY: p2 }), l2 = e4, c2 = n3);
    }, b = (t3) => {
      if (h2.delete(t3.pointerId)) {
        if (t3.pointerId === s2 && u) {
          const e4 = t3.clientX, i3 = t3.clientY;
          o2.set({ type: "end", x: e4 - g, y: i3 - m });
        }
        0 === h2.size && d2();
      }
    }, E = (t3) => {
      t3.relatedTarget && t3.relatedTarget !== document.documentElement || b(t3);
    }, C = (t3) => {
      u && (t3.stopPropagation(), t3.preventDefault());
    }, L = (t3) => {
      t3.defaultPrevented || h2.size > 1 || u && t3.preventDefault();
    };
    document.addEventListener("pointermove", f), document.addEventListener("pointerup", b), document.addEventListener("pointerout", E), document.addEventListener("pointercancel", E), document.addEventListener("touchmove", L, { passive: false }), document.addEventListener("click", C, { capture: true }), d2 = () => {
